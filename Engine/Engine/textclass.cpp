...
bool TextClass::Render(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX orthoMatrix)
{
	bool result;
...
bool TextClass::RenderSentence(ID3D11DeviceContext* deviceContext, SentenceType* sentence, D3DXMATRIX baseWorldMatrix, D3DXMATRIX viewMatrix,
							   D3DXMATRIX orthoMatrix)
{
	...
	result = m_FontShader->Render(deviceContext, sentence->indexCount, baseWorldMatrix, viewMatrix, orthoMatrix, m_Font->GetTexture(),
								  pixelColor);
	if(!result)
	{
		false;
...
bool TextClass::SetCpu(int cpu, ID3D11DeviceContext* deviceContext)
{
	...
	green = 1 - cpu / 100.0f;
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, cpuString, 20, 40, red, green, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
