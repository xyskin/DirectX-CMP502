...
bool LightShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
	...
	float deltavalue, ID3D11ShaderResourceView* texture,float cus)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, lightDirection, diffuseColor,ambientColor, deltavalue,texture,cus);
	if(!result)
	{
		return false;
	}
...
bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
	...
	D3DXVECTOR4 diffuseColor, D3DXVECTOR4 ambientColor, float deltavalue, ID3D11ShaderResourceView* texture,float cus)
{
	HRESULT result;
	...
	dataPtr3->padding = D3DXVECTOR2(); //this is just padding so this data isnt used.
	dataPtr3->cus = cus;
